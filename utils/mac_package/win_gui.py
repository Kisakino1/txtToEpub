...
# 打包成 .app 时若启动崩溃，把错误写到桌面；无错误则不创建该文件
...
# 打包后与 _gui 同目录；开发时在 mac_package 下
if __name__ == '__main__':
    _pkg = os.path.dirname(os.path.abspath(__file__))
    if _pkg not in sys.path:
        sys.path.insert(0, _pkg)

import 检查文件编码_gui as mod_check
import 转换文件编码_gui as mod_convert
import txtToEpub_gui as mod_epub
